...
## พัฒนาต่อ (กติกาของรีโป)

กติกาการพัฒนาทั้งชุดอยู่ที่ [CLAUDE.md](CLAUDE.md) — พอร์ตมาจาก groove-clinic ทั้งก้อน
เปลี่ยนแค่ภาษาโปรแกรม ⇒ ใครเคยทำที่นั่นมาแล้วอ่านที่นี่ได้เลย เลข § ตรงกัน

```bash
bash scripts/verify.sh            # ด่านเดียวที่ต้องเขียวก่อนปิดใบงาน/ก่อน push
bash scripts/task-move.sh NNN     # ย้ายใบงาน todo → done (ใช้ git mv เสมอ)
```

- **ใบงาน**: `tasks/todo/` → `tasks/done/` · ที่ติดคนไม่ใช่ติดโค้ดอยู่ `tasks/todo-human/`
- **การ์ดความรู้**: [.docs/knowledge/index.md](.docs/knowledge/index.md) — อ่านการ์ดของพื้นที่ที่จะแตะก่อนไล่อ่านโค้ด
- **เพดาน 500 บรรทัดต่อไฟล์** · **ทุกเรทอยู่ใน `lib/config-keys.ts`** · **ของที่ตัดสินไม่ได้ต้องขึ้นเป็น warning ห้ามกลายเป็น 0**
- ไฟล์ชั่วคราวลง `.scratch/` เท่านั้น ห้าม `/tmp`
