---
sources:
  # The parse itself: the four buckets, the one-line skip, the dedup, and `calendarDate`.
  - lib/ot-import.ts
  # The pin on all of it (13 since ใบ 072) — the only lane that asserts these buckets.
  ...
  - app/ot/page.tsx
  # Where the three boxes are rendered, one bullet per line — the claim that no new error surface
  # was needed rests on this file, not on the parse.
  - app/ot/_components/paste-form.tsx
---

# การวางทับ OT ทีละหลายบรรทัด — สี่ถัง และทำไมทรานแซกชันต้องมาทีหลังด่าน
...
paste, which refuses **per line** into three boxes instead of one `?err=` slot, and whose write is
all-or-nothing.
